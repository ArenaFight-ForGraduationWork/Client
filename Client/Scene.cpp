...
{
	CScene *pScene;
	for (BYTE i = (BYTE)eSceneMode::FIRST; i < (BYTE)eSceneMode::END; ++i)
	{
		m_mScenes[(eSceneMode)i] = new CScene();
	}

	m_eNowPlayingScene = eSceneMode::FIRST;
}
CSceneManager::~CSceneManager()
{
}
...
void CSceneManager::Render(ID3D11DeviceContext *pd3dDeviceContext)
{
	m_mScenes[m_eNowPlayingScene]->Render(pd3dDeviceContext);
}

void CSceneManager::BuildObjects(ID3D11Device *pd3dDevice)
{
	m_mScenes[m_eNowPlayingScene]->BuildObjects(pd3dDevice);
}

void CSceneManager::AnimateObjects(float fTimeElapsed)
{
	m_mScenes[m_eNowPlayingScene]->AnimateObjects(fTimeElapsed);
}

void CSceneManager::ReleaseObjectsInNowScene()
{
	m_mScenes[m_eNowPlayingScene]->ReleaseObjects();
}
void CSceneManager::ReleaseObjectsInAllScene()
{
	for (BYTE i = (BYTE)eSceneMode::FIRST; i < (BYTE)eSceneMode::END; ++i)
	{
		m_mScenes[(eSceneMode)i]->ReleaseObjects();
	}
}
void CSceneManager::Destroy()
{
	ReleaseObjectsInAllScene();
