...
void CFirstScene::BuildObjects(ID3D11Device *pd3dDevice)
{
	...
	m_pPlayer->SetObject(m_pObjectManager->Insert(30000, eResourceType::User, pd3dDevice, pd3dDeviceContext, 0, 3, D3DXVECTOR3(0, 0, 0), D3DXVECTOR3(0, 0, 0)));
	m_pPlayer->GetObjects()->SetPlayAnimationState(ePLAYER_STATE::IDLE);

	m_pCameraManager = CCameraManager::GetSingleton();
	m_pCameraManager->GetNowCamera()->SetLookAt(m_pPlayer->GetPosition());

	m_pObjectManager->Insert(20000, eResourceType::Bird, pd3dDevice, pd3dDeviceContext, 3, 3, D3DXVECTOR3(0, 0, 0), D3DXVECTOR3(0, 0, 0));
	m_pObjectManager->Insert(10, eResourceType::Floor, D3DXVECTOR3(0, 0, 0));
	for (int i = 0; i < 6; ++i)
		m_pObjectManager->Insert(i, eResourceType::Item_HP, D3DXVECTOR3(120 * i, 0, 100));
}
