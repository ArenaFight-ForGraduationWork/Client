...
void CParticle::Initialize(char *pShaderFileName, wstring DdsFileName, UINT uiMaxParticles)
{
	...
	HR(gpCommonState->GetDevice()->CreateTexture2D(&texArrayDesc, 0, &texArray));
	...
	for (UINT texElement = 0; texElement < size; ++texElement)	// for each texture element...	
	{
		for (UINT mipLevel = 0; mipLevel < texElementDesc.MipLevels; ++mipLevel)	// for each mipmap level...
		{
			...
			gpCommonState->GetDeviceContext()->CopySubresourceRegion(texArray, static_cast<uint32_t> (destinationSubresource),
	...
	HR(gpCommonState->GetDevice()->CreateShaderResourceView(texArray, &viewDesc, &texArraySRV));
	...
	HR(gpCommonState->GetDevice()->CreateTexture1D(&texDesc, &initData, &randomTex));
	...
	HR(gpCommonState->GetDevice()->CreateShaderResourceView(randomTex, &viewDesc1, &randomTexSRV));
	randomTex->Release();
...
void CParticle::Draw(CXMMATRIX viewMatrix, CXMMATRIX projectionMatrix)
{
	...
	gpCommonState->GetDeviceContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_POINTLIST);
	...
	if (mFirstRun)
		gpCommonState->GetDeviceContext()->IASetVertexBuffers(0, 1, &mInitVB, &stride, &offset);
	else
		gpCommonState->GetDeviceContext()->IASetVertexBuffers(0, 1, &mDrawVB, &stride, &offset);

	//
	// Draw the current particle list using stream-out only to update them.  
	// The updated vertices are streamed-out to the target VB. 
	//
	gpCommonState->GetDeviceContext()->SOSetTargets(1, &mStreamOutVB, &offset);

	D3DX11_TECHNIQUE_DESC techDesc;
	m_pShader->m_particle->StreamOutTech->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		HR(m_pShader->m_particle->StreamOutTech->GetPassByIndex(p)->Apply(0, gpCommonState->GetDeviceContext()));

		if (mFirstRun)
		{
			gpCommonState->GetDeviceContext()->Draw(1, 0);
			mFirstRun = false;
		}
		else
		{
			gpCommonState->GetDeviceContext()->DrawAuto();
		}
	}

	// done streaming-out--unbind the vertex buffer
	ID3D11Buffer* bufferArray[1] = { 0 };
	gpCommonState->GetDeviceContext()->SOSetTargets(1, bufferArray, &offset);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		HR(m_pShader->m_particle->DrawTech->GetPassByIndex(p)->Apply(0, gpCommonState->GetDeviceContext()));
		gpCommonState->GetDeviceContext()->DrawAuto();
	}
...
void CParticle::BuildVB()
{
	...
	HR(gpCommonState->GetDevice()->CreateBuffer(&vbd, &vinitData, &mInitVB));
	...
	HR(gpCommonState->GetDevice()->CreateBuffer(&vbd, 0, &mDrawVB));
	HR(gpCommonState->GetDevice()->CreateBuffer(&vbd, 0, &mStreamOutVB));
}